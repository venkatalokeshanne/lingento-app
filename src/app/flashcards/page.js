...
export default function FlashcardsPage() {
  ...
  // Fetch flashcards from vocabulary when user is available
  useEffect(() => {
    if (currentUser) {
      fetchFlashcards();
    }
  }, [currentUser]);  // Fetch vocabulary and convert to flashcards using utility function
  const fetchFlashcards = async () => {
    try {
      const flashcardsData = await fetchVocabularyAsFlashcards(currentUser, setAllFlashcards, setLoading, setError);
  ...
  const handleQualityRating = async (cardId, quality) => {
    try {
      ...
      const updatedCard = spacedRepetitionService.calculateNextReview(currentCard, quality);
      ...
      <div className="max-w-screen-md mx-auto">
        ...
        {error && !loading && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-medium text-gray-700 dark:text-gray-300 mb-2">Unable to load flashcards</h3>
            <p className="text-gray-500 dark:text-gray-400 mb-4">{error}</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">Showing sample data instead</p>
          </div>
        )}
            ...
            <>
              ...
              >
                <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                  ...
                  <div className="mb-4">
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Study Mode</p>
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      {[
                        { value: 'review', label: 'Review', desc: 'Due cards' },
                  ...
                  {/* Category Selection */}
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Select category</p>
                  ...
                  <div className="mt-4 bg-gray-50 dark:bg-gray-700/30 px-3 py-2 rounded-lg space-y-2">
                    <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span>Learned: {allFlashcards.filter(card => card.mastered).length}</span>
        ...
        </AnimatePresence>          {/* We removed the floating progress bar since we integrated it into the button */}        {/* Circular Progress Filter Button */}
        <div className="fixed bottom-6 right-6 z-50">
          <motion.button
            ...
            style={{
              animation: isPulsing ? 'pulsateProgress 1.5s ease-out' : 'none'
            }}
            whileHover={{ 
              scale: 1.1, 
            ...
            animate={{ 
              ...
              boxShadow: selectedCategory !== 'All' 
                ? ["0 4px 12px rgba(79, 70, 229, 0.5)", "0 8px 20px rgba(79, 70, 229, 0.7)", "0 4px 12px rgba(79, 70, 229, 0.5)"]
            ...
            key={`button-${allFlashcards.filter(card => card.mastered).length}`}
          >
            ...
            <svg className="absolute inset-0 w-full h-full -rotate-90">
              ...
              <motion.circle
                ...
                animate={{ 
                  strokeDashoffset: allFlashcards.length > 0 
                    ? 139 * (1 - (allFlashcards.filter(card => card.mastered).length / allFlashcards.length))
                    : 139 
                }}
                transition={{ duration: 0.8, type: "spring" }}
                key={`circle-progress-${allFlashcards.filter(card => card.mastered).length}`}
              />
            </svg>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div 
                ...
                key={`progress-${allFlashcards.filter(card => card.mastered).length}`}
              >
                {allFlashcards.length > 0 
                  ? Math.round((allFlashcards.filter(card => card.mastered).length / allFlashcards.length) * 100) 
                  : 0}%
              </div>
            </div>
        ...
        <AnimatePresence>
          ...
          <div className="text-center py-10">
            ...
            <h3 className="text-xl font-medium text-gray-700 dark:text-gray-300 mt-4">No flashcards match your filters</h3>
            <p className="text-gray-500 dark:text-gray-400 mt-2">Try changing your filter settings or category selection</p>
          </div>
            ...
            <div className="fixed bottom-4 sm:bottom-8 left-1/2 transform -translate-x-1/2 flex justify-center items-center gap-3 sm:gap-4 z-20 p-2">
              {/* Previous button */}              <motion.button
                ...
                onTapStart={() => {
                  ...
                  document.querySelector('.prev-btn-container')?.appendChild(ripple);
              ...
              >
                ...
                <span className="relative z-10 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-indigo-600 dark:text-indigo-400 transform transition-transform duration-200 group-hover:-translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              >
                <span className={`absolute inset-0 rounded-full shadow-lg transform transition-all duration-200 ${
                ...
                <span className="relative z-10 flex items-center justify-center">
                  {filteredFlashcards[activeIndex]?.mastered ? (
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-gray-500 dark:text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <motion.button
                ...
                onTapStart={() => {
                  ...
                  ripple.style.pointerEvents = 'none';
                  ripple.style.zIndex = '0';
                  document.querySelector('.next-btn-container')?.appendChild(ripple);
              ...
              >
                ...
                {/* Icon container */}
                <span className="relative z-10 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white transform transition-transform duration-200 group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </span>              </motion.button>            </div>
          </>
        ) : (
          <div className="text-center py-10">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4 mx-auto"></div>
